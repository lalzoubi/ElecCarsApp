...
public class UserInfoController {
    ...
    public ResponseEntity<Object> registerUser(@RequestBody @Valid UserInfoReqDto user) {

        Optional<UserInfoResDto> info = service.findUser(user.getUsername());
        if (info.isPresent()) {
    ...
    public ResponseEntity<Object> loginUser(@RequestParam("username") String username, @RequestParam("password") String password) {
        ...
        if (verifyUserResponse != null) {
            UserInfoResDto userInfoResponse = service.fetchUserDetails(username);
    ...
    public ResponseEntity<?> getUserByIdentifier(@RequestParam("id") String id) {
        Optional<UserInfoResDto> info = service.findUser(id);
        if (info.isPresent()) {
            return ApiCallResponse.generateResponse(1, "تم استرجاع البيانات بنجاح", "The data retrieved successfully", HttpStatus.OK, info);
        }
    ...
    public ResponseEntity<?> getIsActiveUser(@RequestParam("username") String username) {
        ...
        Optional<UserInfoResDto> info = service.findUser(username);

        if (info.isPresent()) {
            UserInfoResDto userInfoResponse = service.findByIsActiveTrue(username);
            if (userInfoResponse != null)
    ...
    public ResponseEntity<?> getIsConfirmedUserByIdentifier(@RequestParam("username") String username) {
        ...
        if (info.isPresent()) {
            UserInfoResDto userInfoResponse = service.findByIsUserConfirmedTrue(username);
            if (userInfoResponse != null)
